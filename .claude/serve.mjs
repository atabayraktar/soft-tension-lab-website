...
const server = http.createServer((req, res) => {
  // req.url is the raw request-target — percent-encoded (e.g. a dynamic [slug].js chunk
  // arrives as %5Bslug%5D.js) and never decoded automatically by Node's http module, so
  // file lookups against it silently 404 without this.
  let urlPath = decodeURIComponent(req.url.split('?')[0]);
  if (urlPath === '/') urlPath = '/index.html';
  else if (urlPath.endsWith('/')) urlPath += 'index.html';

  let filePath = path.join(ROOT, urlPath);
  // Next's static export writes route directories (e.g. /about -> out/about/index.html)
