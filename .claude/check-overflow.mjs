...
// Optional base URL argument (default http://localhost:3000), e.g. `node .claude/check-overflow.mjs http://localhost:3001`
const BASE = (process.argv[2] || 'http://localhost:3000').replace(/\/$/, '');
const PAGES = ['', '/about/', '/work/', '/services/', '/contact/', '/shop/', '/holding/', '/404/'].map((p) => BASE + p);
const VIEWPORTS = [
